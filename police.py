...
query_map = {
    ...
    "Count of Stops by Violation Type": "SELECT violation_type, COUNT(*) AS count FROM police_logs GROUP BY violation_type",
    "Number of Arrests vs. Warnings": "SELECT action_taken, COUNT(*) AS count FROM police_logs GROUP BY action_taken",
    "Average Age of Drivers Stopped": "SELECT AVG(driver_age) AS avg_age FROM police_logs",
    "Top 5 Most Frequent Search Types": "SELECT search_type, COUNT(*) AS count FROM police_logs GROUP BY search_type ORDER BY count DESC LIMIT 5",
    "Count of Stops by Gender": "SELECT driver_gender, COUNT(*) AS count FROM police_logs GROUP BY driver_gender",
    "Most Common Violation for Arrests": "SELECT violation_type, COUNT(*) AS count FROM police_logs WHERE action_taken='Arrest' GROUP BY violation_type ORDER BY count DESC LIMIT 1"
}
